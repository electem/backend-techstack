...
!function ($) {
    ...
    BootstrapTable.prototype.initToolbar = function () {
        ...
        if (typeof this.options.toolbar === 'string') { // col-* doubled for grid24
            $(sprintf('<div class="col-lg-18 col-md-16 col-sm-14 col-xs-16" style="padding: 4px 0 8px 0;"></div>', this.options.toolbarAlign)) //<div class="bars pull-%s"></div>
                .appendTo(this.$toolbar)
                .append($(this.options.toolbar));
        }

        // showColumns, showToggle, showRefresh  // col-* doubled for grid24
        html = [sprintf('<div id="try" class="col-lg-6 col-md-8 col-sm-10 col-xs-8 pull-right" style="text-align: right; border: 0px solid blue; padding: 4px 0 8px 0;"><div class="btn-group">',//<div class="btn-group">
        ...
        if (this.options.showToggle) {
            html.push(sprintf('<button id="toggle" class="btn btn-default' + (this.options.iconSize === undefined ? '' : ' btn-' + this.options.iconSize) + '" type="button" name="toggle" title="%s">',
        ...
        if (this.options.showColumns) {
            html.push(sprintf('<div id="columns" class="keep-open btn-group" title="%s">',
                this.options.formatColumns()),
                '<button type="button" class="btn btn-default' + (this.options.iconSize == undefined ? '' : ' btn-' + this.options.iconSize) + ' dropdown-toggle" data-toggle="dropdown">',
                sprintf('<i class="%s %s"></i>', this.options.iconsPrefix, this.options.icons.columns), // actual toggle button
                ' <span class="caret"></span>',
                '</button>',
                '<ul style="padding: 10px;" class="dropdown-menu" role="menu">');
        ...
        for (var i = this.pageFrom - 1; i < this.pageTo; i++) {
            ...
            $.each(this.header.fields, function (j, field) {
                ...
                if (column.checkbox || column.radio) {
                    ...
                    text = [that.options.cardView ? // regular view
                        '<div class="col-xs-4"/* cardview */ style="float: left; margin: 0; padding: 0;/* cardview */"><div style="height: 90px; width: 100%; padding: 0; margin: 0; background: gray; vertical-align: middle;/* cardview */">' : '<td>'/* regular view */,
                        '<div style="display: inline; margin: auto; padding: 0;"><input style="width: 24px; height: 24px; display: inline !important; position: relative !important; top: 35% !important; margin: auto !important; padding: 0 !important;"' +/* both views */
                            sprintf(' data-index="%s"', i) +
                            ...
                            ' /><i style="color: #DEDEDE;" class="cardview-arrow-hidden fa icon-V Fs16"></i></div>',
                        that.options.cardView ? '</div></div>' : '</td>'].join('');
                    ...
                    text = that.options.cardView ? // cardview main cell
                        ['<div class="col-xs-20"/* cardview */ style="float: right; padding: 0; margin: 0;/* cardview */"><div style="border-bottom: 1px dashed #FFF !important; vertical-align: middle !important;">'/* cardview */,
                            that.options.showHeader ? sprintf('<span style="text-align: left;" class="title col-xs-12" %s>%s</span>', style,/* cardview */
                                getPropertyFromOther(that.options.columns, 'field', 'title', field)) : '',/* cardview */
                            sprintf('<span style="text-align: left;" class="value col-xs-12">%s</span>', value),/* cardview */
                            '</div></div>'].join('') :
                        [sprintf('<td%s %s %s %s %s>', id_, class_, style, data_, rowspan_),/* regular view */
                            value,
                            '</td>'].join('');
...
!function($) {
    ...
    BootstrapTable.prototype.initToolbar = function() {
        ...
        html.push(sprintf('<div class="btn-group" role="group">', this.options.buttonsAlign, this.options.buttonsAlign));
