...
const loginUrl = 'http://localhost:8080/adduser';
...
export class TutorialService {
  ...
  private roles = [{ role: 'Author' }, { role: 'Comment' }];
  ...
  async createTutorial(tutorial: Tutorial): Promise<any> {
    this.tokenIs = localStorage.getItem('id_token')!;
    const header = new HttpHeaders().set(
      'Authorization',
      'Bearer ' + this.tokenIs
    );
    return await this.http.post(url, tutorial, { headers: header }).toPromise();
  }
  ...
  async createComment(comment: Comments): Promise<Comments> {
    ...
    return await this.http
      .post(commentUrl, comment, { headers: header })
      .toPromise();
  }
  ...
  async getAllCommentsBasedOnTutorilID(id: Number) {
    ...
    return await this.http
      .get(`${commentUrlGet}/${id}`, { headers: header })
      .toPromise();
  }

  async createlogin(user: UserLogin): Promise<UserLogin> {
    this.tokenIs = localStorage.getItem('id_token')!;
    const header = new HttpHeaders().set(
      'Authorization',
      'Bearer ' + this.tokenIs
    ...
    return await this.http
      .post(loginUrl, user, { headers: header })
      .toPromise();
  }
