...
public class ArticleApiTest extends TestWithCurrentUser {
  ...
  public void should_read_article_success() throws Exception {
    ...
    RestAssuredMockMvc.when()
        ...
        .body("article.header", equalTo(articleData.getHeader()))
  ...
  public void should_update_article_content_success() throws Exception {
    ...
    Map<String, Object> updateParam = prepareUpdateParam(title, body,header, description);

    Article article =
        new Article(title, description, body,header, Arrays.asList("java", "spring", "jpg"), user.getId());
  ...
  public void should_get_403_if_not_author_to_update_article() throws Exception {
    ...
    Article article =
        new Article(
            title, description, body, header,Arrays.asList("java", "spring", "jpg","jpgg"), anotherUser.getId());
  ...
  public void should_delete_article_success() throws Exception {
    ...
    Article article =
        new Article(title, description, body, header,Arrays.asList("java", "spring", "jpg","javava"), user.getId());
    when(articleRepository.findBySlug(eq(article.getSlug()))).thenReturn(Optional.of(article));
  ...
  public void should_403_if_not_author_delete_article() throws Exception {
    ...
    Article article =
        new Article(
            title, description, body,header, Arrays.asList("java", "spring", "jpg","javava"), anotherUser.getId());
  ...
  private HashMap<String, Object> prepareUpdateParam(
      final String title, final String body, String header,final String description) {
